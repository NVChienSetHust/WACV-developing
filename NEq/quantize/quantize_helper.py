...
def create_scaled_head(model_q, norm_feat=False):
    ...
    model_q[-2] = ScaledLinear(model_q[-2].in_channels, config.data_provider.new_num_classes,
                               model_q[-2].x_scale, model_q[-2].zero_x, norm_feat=norm_feat)
    # model_q[-1] = ScaledLinear(model_q[-1].in_channels, config.data_provider.new_num_classes,
    #                            model_q[-1].x_scale, model_q[-1].zero_x, norm_feat=norm_feat)
    return model_q
...
def create_quantized_head(model_q):
    ...
    if not isinstance(model_q[-1], nn.Flatten):
        model_q = _append_flatten(model_q)
    sample_linear = nn.Conv2d(model_q[-2].in_channels, config.data_provider.new_num_classes, 1)
    ...
    model_q[-2] = QuantizedConv2dDiff(model_q[-2].in_channels, config.data_provider.new_num_classes, 1,
